...
//      ==> 데이터베이스에 접근하는 객체 
	...
	public List<MemoDTO> selectAllMemo() throws SQLException {
		...
		// >>> 3. SQL문(쿼리문, 편지) 작성하기 <<<
		String sql = "SELECT no, name, msg FROM tbl_memo ORDER BY no DESC";
	...
	public List<MemoDTO> searchMemoByWord(String searchword) throws SQLException {
		...
		while(rs.next()) {
			...
			MemoDTO dto = new MemoDTO(rs.getInt("no"), rs.getString("name"), rs.getString("msg")); 
			memoList.add(dto);
		}
	...
	public MemoDTO searchMemoByNo(String no) throws SQLException {
	    ...
	    if (rs.next()) {
	    	mdto = new MemoDTO(rs.getInt("no"), rs.getString("name"), rs.getString("msg")); 
	    }
	...
	public List<MemoDTO> selectByDate(int day) throws SQLException {
	  ...
	  // >>> 3. SQL문(쿼리문, 편지) 작성하기 <<<
		 String sql = "SELECT no, " 
				 ...
				 +"FROM tbl_memo " 
				 +"WHERE to_date(TO_CHAR(sysdate,'yyyy-mm-dd'),'yyyy-mm-dd') - to_date(TO_CHAR(writeday,'yyyy-mm-dd'),'yyyy-mm-dd') <= ? " 
				 +"ORDER BY no DESC";
	  ...
	  // >>> 5. SQL구문 실행하기 <<<
	     ...
	     while(rs.next()) {
	    	 ...
	    	 MemoDTO mdto = new MemoDTO(rs.getInt("no"), rs.getString("name"), rs.getString("msg"), rs.getString("WRITEDAY"));
	    	 list.add(mdto);
	     }
	...
	public List<MemoDTO> selectByDate(String startday, String endday) throws SQLException {
		...
		String sql = "SELECT no, " 
				...
				+"  TO_CHAR(writeday, 'yyyy-mm-dd hh24:mi:ss') AS WRITEDAY " 
				+"FROM tbl_memo " 
				+"WHERE TO_CHAR(writeday,'yyyy-mm-dd') BETWEEN ? AND ? " 
				+"ORDER BY no DESC";
